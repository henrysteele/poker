...
export function DeckOfCards(props) {
  ...
  createEffect(() => {
    ...
    const dummies =
      props.discard && props.cards.length < 10 ? [] : "#####".split("")
      ...
      <div style={{ position: "relative", height: "100px" }}>
        <For each={props.cards}>
          {(card, i) => {
            ...
            return (
              <PlayingCard id={card} style={style} down={false}></PlayingCard>
            )
          }}
        </For>
      </div>
          ...
          {(row) => (
            <>
              <Stack direction="row" spacing={1}>
                <For each={row}>{(item) => <>{props.render(item)}</>}</For>
              </Stack>
            </>
          )}
        </For>
  ...
  return (
    <Box id={props.id || "hand"} alignContent="center">
      ...
      <Show when={false && props.cards?.length == 5}>
        <Box>rank: {Math.round(getRank(props.cards).score * 1000) / 1000}</Box>{" "}
